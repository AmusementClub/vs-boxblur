    ...
    if (activationReason == arInitial) {
        ...
        for (int plane = 0; plane < vi->format->numPlanes; ++plane) {
            if (d->radius[plane] > 0) {
                ...
                if (bytes == 4) {
                    ispc::blurVF((float *) dstp, (const float *) srcp, width, height, stride, d->radius[plane], (float *) buffer);
                } else if (bytes == 2) {
                    ispc::blurVUS((uint16_t *) dstp, (const uint16_t *) srcp, width, height, stride, d->radius[plane], (uint32_t *) buffer);
                } else if (bytes == 1) {
                    ispc::blurVUC((uint8_t *) dstp, (const uint8_t *) srcp, width, height, stride, d->radius[plane], (uint32_t *) buffer);
                }
            }
        }
...
VS_EXTERNAL_API(void) VapourSynthPluginInit(
    ...
    configFunc("io.github.amusementclub.boxblur_ispc", "box_ispc", "AVX2-optimized boxfilter", VAPOURSYNTH_API_VERSION, 1, plugin);
